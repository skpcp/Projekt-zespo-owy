...
Metoda | Funkcjonalność | Opis 
 --- | --- | --- 
*/updateProjectsForUser* | Update project list for user | Pozwala na pobranie odpowiedniego projektu dla odpowiedniego użytkownika. Aby tego dokonać należy podać numer id użytkownika, któremu chcemy zapisać dany projekt i nazwę projektu w polu project (projekt). Jakie kolwiek błędy zostaną pokazane na samym końcu akcji.
*/updatePermissionsForUser* | Update permission list for user | Pozwala na pobranie odpowiedniego pozwolenia dla odpowiedniego użytkownika. Aby tego dokonać należy podać numer id użytkownika, któremu chcemy zapisać dane pozwolenie i nazwę pozwolenia w polu pozwolenie (permission). Jakie kolwiek błędy zostaną pokazane na samym końcu akcji.
...
*/getByActivity* | Find user by activity | Pozwala na wyszukanie użytkowników z odpowiednią aktywnością. W polu active mamy do wyboru wartości true lub false. Jaki kolwiek błąd zostanie pokazany na ekranie naszego systemu.
...
*/changeActivity* | Update user activity | Pozwala na zmianę aktywności użytkownika. Domyślna wartość to true. Zmiany można dokonać po przez podanie aktywności i przypisanie jej do numeru id użytkownika, któremu chcemy ją zmienić. Jeżeli w trakcie zostanie popełniony błąd system nam to pokaże w komunikacie. Zmiana aktywności może oznaczać że użytkownik jest do usunięcia z systemu.
</br>
